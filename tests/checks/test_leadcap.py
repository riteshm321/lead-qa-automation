...
def test_flat_leadcap_passes_when_count_meets_cap():
...
def test_flat_leadcap_fails_when_count_exceeds_cap():
    config = LeadcapConfig(enabled=True, segmented=False, flat_cap=2)
    new_leads = pd.DataFrame([{"CID": "118118", "emailaddress": "a@x.com"}])
    purchased = pd.DataFrame({
        "Campaign ID": ["118118", "118118", "118118"],
        "Email": ["p1@x.com", "p2@x.com", "p3@x.com"],
    })
...
def test_segmented_leadcap_pools_cap_across_cids_in_segment():
    ...
    purchased = pd.DataFrame({
        "Campaign ID": ["98778", "98778", "98778", "98779", "98779", "98779"],
        "Email": ["p1@x.com", "p2@x.com", "p3@x.com", "p4@x.com", "p5@x.com", "p6@x.com"],
    })
...
def test_leadcap_fails_when_domain_specific_count_exceeds_cap():
    config = LeadcapConfig(enabled=True, segmented=True, segments=[
        LeadcapSegment(name="Shared Pair", cids=["98778", "98779"], cap=5),
    ])
    purchased = pd.DataFrame({
        "Campaign ID": ["98778", "98778", "98778", "98778", "98778", "98778", "98779"],
        "Email": [
            "a1@heavydomain.com", "a2@heavydomain.com", "a3@heavydomain.com",
            "a4@heavydomain.com", "a5@heavydomain.com", "a6@heavydomain.com", "b1@lightdomain.com",
        ],
    })

    new_leads = pd.DataFrame([
        {"CID": "98779", "emailaddress": "new@lightdomain.com"},  # only 1 prior purchase -> PASS
        {"CID": "98778", "emailaddress": "new@heavydomain.com"},  # 6 prior purchases, cap=5 -> FAIL
    ])
...
def test_leadcap_company_pass_fails_when_passed_domain_but_exceeds_company():
    ...
    purchased = pd.DataFrame({
        "Campaign ID": ["98778", "98778", "98778", "98778", "98779"],
        "Email": ["a1@one.com", "a2@two.com", "a3@three.com", "a4@fourdomain.com", "b1@four.com"],
        "Company": ["Acme Corp", "Acme Corp", "Acme Corp", "Acme Corp", "Other Co"],
    })
    new_leads = pd.DataFrame([
...
def test_leadcap_company_pass_skipped_when_domain_already_failed():
    ...
    purchased = pd.DataFrame({
        "Campaign ID": ["98778", "98778"],
        "Email": ["existing1@samedomain.com", "existing2@samedomain.com"],
        "Company": ["Totally Different Co", "Totally Different Co"],
    })
    new_leads = pd.DataFrame([
...
def test_leadcap_company_match_is_exact_case_insensitive_trimmed():
    ...
    purchased = pd.DataFrame({
        "Campaign ID": ["98778", "98778"],
        "Email": ["existing1@other.com", "existing2@other.com"],
